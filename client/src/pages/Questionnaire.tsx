...
const Questionnaire: React.FC = () => {
    ...
    // Zustand Stores
    const {
        data, currentStep: current, setCurrentStep,
        updateField, submitProfile, clearForm, isLoading, error
    } = useOnboardingStore();
    ...
    const handleBack = () => {
        // Safe return from the summary page back to step 6
        if (done) {
            setDirection(-1);
            setDone(false);
            return;
    ...
    const handleNext = () => {
        // Just show the summary view. No API call here!
    ...
    // The actual API trigger attached to the final summary button
    const handleBuildPlan = async () => {
        try {
            await submitProfile();
            await checkAuth(true); // Force refresh user data
            clearForm(); // Wipes local storage draft
            navigate('/dashboard');
        } catch (err) {
    ...
    // Helper functions mapped to Zustand
    const setAnswer = (key: string, val: number) => updateField(key, val);
    const sliderVal = (key: string, def: number) => data[key] !== undefined ? (data[key] as number) : def;
    const labelOf = (arr: string[], idx: number | undefined) => idx !== undefined ? arr[idx] : '—';

    const toggleMulti = (key: string, val: number) => {
        const cur = (data[key] as number[]) || [];
        if (val === 0) {
            updateField(key, [0]);
            return;
        }
        ...
        updateField(key, nextVal.length === 0 ? [0] : nextVal);
    };
                        ...
                        {done ? (
                            <>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column' }}>
                                    {[
                                        { k: 'Primary Objective', v: labelOf(goalLabels, data.goal as number) },
                                        { k: 'Activity Level', v: labelOf(actLabels, data.activity as number) },
                                        { k: 'Body Mass', v: `${parseFloat(String(data.weight || 70)).toFixed(1)} kg` },
                                        { k: 'Sleep Quality', v: `${Math.round(Number(data.sleep || 7))} / 10` },
                                        { k: 'Eating Pattern', v: labelOf(mealLabels, data.meals as number) },
                                        { k: 'Dietary Needs', v: ((data.restrictions as number[]) || []).map(i => dietLabels[i]).join(', ') || 'None' },
                                    ].map(row => (
                                ...
                                {/* Display errors on the summary page if the API fails */}
                                {error && <div style={{ color: '#ef4444', fontSize: '0.85rem', marginTop: '1.5rem', fontWeight: 500, textAlign: 'center' }}>{error}</div>}
                                ...
                                <button
                                    ...
                                    onMouseEnter={e => { if (!isLoading) (e.currentTarget as HTMLButtonElement).style.background = colors.emeraldLight; }}
                                    onMouseLeave={e => { if (!isLoading) (e.currentTarget as HTMLButtonElement).style.background = colors.emerald; }}
                                >
                                    {isLoading ? (
                            ...
                            <>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: '2rem' }}>
                                        {step.options.map((o, i) => (
                                            <OptionCard key={i} icon={o.icon} label={o.label} desc={o.desc}
                                                colors={colors} selected={data[step.key] === i} onClick={() => setAnswer(step.key, i)} />
                                        ))}
                                    </div>
                                )}
                                ...
                                {step.type === 'multi' && (
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: '2rem' }}>
                                        {step.options.map((o, i) => (
                                            <OptionCard key={i} icon={o.icon} label={o.label} desc={o.desc}
                                                colors={colors}
                                                selected={((data[step.key] as number[]) || []).includes(i)}
                                ...
                                <button
                                    ...
                                    style={{
                                        ...
                                        background: canProceed(step, data) ? colors.emerald : colors.bgElevated,
                                        color: canProceed(step, data) ? colors.bgPage : colors.textMuted,
                                        fontSize: '0.76rem', fontWeight: 800, letterSpacing: '0.16em', textTransform: 'uppercase',
                                        border: `0.5px solid ${canProceed(step, data) ? colors.emeraldBorder : colors.borderSubtle}`,
                                        borderRadius: 10, cursor: canProceed(step, data) ? 'pointer' : 'not-allowed',
                                        transition: 'all 0.18s',
                                        display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px'
                                    }}
                                    onMouseEnter={e => { if (canProceed(step, data)) (e.currentTarget as HTMLButtonElement).style.background = colors.emeraldLight; }}
                                    onMouseLeave={e => { if (canProceed(step, data)) (e.currentTarget as HTMLButtonElement).style.background = colors.emerald; }}
                                >
                                    {current === STEPS.length - 1 ? 'Review Profile →' : 'Continue →'}
                                </button>
                            </>
                        )}
                    </motion.div>
